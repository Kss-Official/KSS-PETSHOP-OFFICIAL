...
export const PharmacyConcernPage: React.FC = () => {
  ...
  // Resolve health concern metadata
  const currentConcern = useMemo(() => {
    return resolveConcernFromSlug(concernSlug);
  }, [concernSlug]);
  ...
  // Filter products strictly for this health concern using keyword matching
  const concernProducts = useMemo(() => {
    if (!currentConcern) return [];

    return products.filter((p) => {
      // Must match health concern keywords
      if (!matchesHealthConcern(p, currentConcern.slug)) return false;
  ...
  }, [products, currentConcern, searchQuery, maxPrice, prescriptionOnly]);

  // Sorted products
  const sortedProducts = useMemo(() => {
    const list = [...concernProducts];
    if (sortBy === 'price-asc') {
  ...
  // If the slug did not match any known health concern
      ...
      <div className="min-h-screen bg-[#FAF6EE] text-[#16241B] font-sans flex flex-col">
        ...
        <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <EmptyState
            ...
            description="The pet health concern collection you requested does not exist. Browse our verified pharmacy categories and concerns below."
      ...
      <main className="flex-grow space-y-6 sm:space-y-8 pb-14 pt-4 sm:pt-6">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 text-xs sm:text-sm text-[#556658] font-medium mb-3">
            ...
            <span className="text-[#88998C]">/</span>
            <span className="text-[#16241B] font-bold">{currentConcern?.title}</span>
          </nav>
        ...
        {/* 4. Products Grid Scoped to this Health Concern */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? (
            ...
            <div className="py-6 sm:py-8">
              <EmptyState
                title={`No products found for ${currentConcern?.title} yet`}
                description="We are regularly updating our veterinary inventory with specialized formulations. Try exploring all pharmacy products or clearing active filters."
                actionLabel="Return to Pharmacy"
                onAction={() => navigate('/pharmacy')}
              />
            </div>
        ...
        {/* 5. Explore Other Pharmacy Categories Section */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2">
          <div className="bg-[#F6F7F2] rounded-[32px] p-5 sm:p-7 border border-[#CBDAC6]/60 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {PHARMACY_CATEGORIES.filter((c) =>
                ['medications', 'food-and-nutrition', 'supplements-and-care', 'flea-and-tick'].includes(c.slug)
              ).map((cat) => {
                ...
                return (
                  <Link
                    ...
                    className="group bg-white rounded-2xl p-5 border border-[#16241B]/8 shadow-2xs hover:shadow-md hover:border-[#009E66]/40 transition-all flex flex-col items-start text-left"
                  >
                    <div
                      className={`w-10 h-10 rounded-xl ${cat.color.iconBg} ${cat.color.iconText} flex items-center justify-center mb-3 group-hover:scale-105 transition-transform`}
                    >
                      <IconComp className="w-5 h-5" />
                    </div>
                    <h4 className="font-bold text-sm sm:text-base text-[#16241B] group-hover:text-[#009E66] transition-colors leading-snug">
                      {cat.name}
                    </h4>
                    <p className="text-xs text-[#556658] mt-1 line-clamp-2 font-normal leading-relaxed">
                      {cat.tagline}
                    </p>
                  </Link>
                );
...
export default PharmacyConcernPage;
